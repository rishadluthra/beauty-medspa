...
export function AppointmentStatusChart() {
  ...
  if (isLoading) return <p className="text-brand-bg/70">Loading appointment status…</p>;
  if (!data || data.length === 0) return <p className="text-brand-bg/70">No appointment data yet.</p>;
        ...
        <PieChart>
          <Pie data={chartData} dataKey="count" nameKey="status" outerRadius={100} label>
            {/*
              ...
              is keyed by the raw lowercase strings ("confirmed", "pending",
              "cancelled"), so the lookup here must use `entry.status` from
              the unformatted array. If this were changed to map over
              `chartData` instead, every lookup would be against formatted
              text like "Confirmed" — which doesn't match any STATUS_COLORS
              key — and every slice would silently fall through to the gray
              fallback. Do not "simplify" this to a single shared array.
            */}
            {data.map((entry) => (
