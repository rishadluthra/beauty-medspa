...
from app.routers import analytics, appointments, availability, custom_reports, custom_views, patients, providers, services
...
# route is GET-only EXCEPT `app.routers.custom_reports`/`custom_views` (the
# self-serve analytics feature's saved-report/saved-view CRUD and reorder
# endpoints, POST/PUT/DELETE) — the spec's read-only note is scope relief
# on the seed *business* data, not a blanket prohibition on app-level
# state, so these methods are widened in specifically to support that
# feature. `cors_origins` is a comma-separated string (env var) so it can
# be set to just the deployed Next.js frontend's URL in production while
